v {xschem version=3.4.8RC file_version=1.2}
G {}
K {}
V {}
S {}
F {}
E {}
B 2 70 -1775 870 -1375 {flags=graph
y1=0
y2=1.3
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=1.5e-09
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0
color=12
node=vinI
linewidth_mult=5}
B 2 70 -1335 870 -935 {flags=graph
y1=0
y2=1.3
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=1.5e-09
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0
color=7
node=vinQ
linewidth_mult=5}
B 2 890 -1775 1690 -1375 {flags=graph
y1=0
y2=1.3
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=1.5e-09
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0
color=12
node=vinIB
linewidth_mult=5}
B 2 890 -1335 1690 -935 {flags=graph
y1=0
y2=1.3
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=1.5e-09
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0
color=7
node=vinQB
linewidth_mult=5}
B 2 2620 -1815 4470 -1055 {flags=graph
y1=-0.029
y2=1.3
ypos1=0
ypos2=2
divy=5
subdivy=1
unity=1
x1=0
x2=1.5e-09
divx=5
subdivx=1


dataset=-1
unitx=1
logx=0
logy=0
color="12 7 9 11 10"
node="Vout8I
Vout5I3Q
Vout4I4Q
Vout3I5Q
Vout8Q"
linewidth_mult=5
hilight_wave=-1}
N 120 -500 120 -480 {
lab=GND}
N 120 -620 120 -560 {
lab=vinI}
N 100 -620 120 -620 {
lab=vinI}
N 120 -310 120 -290 {
lab=GND}
N 120 -430 120 -370 {
lab=vinQ}
N 100 -430 120 -430 {
lab=vinQ}
N 440 -500 440 -480 {
lab=GND}
N 440 -620 440 -560 {
lab=vinIB}
N 420 -620 440 -620 {
lab=vinIB}
N 440 -430 440 -370 {
lab=vinQB}
N 420 -430 440 -430 {
lab=vinQB}
N 300 -260 300 -220 {lab=VDD}
N 130 -170 150 -170 {
lab=vinI}
N 130 -150 150 -150 {
lab=vinQ}
N 130 -130 150 -130 {
lab=vinIB}
N 130 -110 150 -110 {
lab=vinQB}
N 450 -170 480 -170 {
lab=VoutI[4:0]}
N 450 -150 480 -150 {
lab=VoutQ[4:0]}
N 450 -130 480 -130 {
lab=VoutIB[4:0]}
N 450 -110 480 -110 {
lab=VoutQB[4:0]}
N 440 -310 440 -290 {
lab=GND}
N 320 -830 320 -790 {lab=VDD}
N 120 -830 120 -790 {lab=v0}
N 220 -830 220 -790 {lab=v1}
N 420 -730 420 -670 {
lab=GND}
N 420 -830 420 -790 {lab=VSS}
N 120 -730 120 -690 {lab=VSS}
N 220 -730 220 -690 {lab=VSS}
N 320 -730 320 -690 {lab=VSS}
N 300 -60 300 -20 {lab=VSS}
N 1010 -780 1040 -780 {
lab=VoutI[0]}
N 1010 -700 1040 -700 {
lab=VoutQ[0]}
N 1010 -740 1040 -740 {
lab=VoutIB[0]}
N 1010 -660 1040 -660 {
lab=VoutQB[0]}
N 1260 -590 1260 -550 {lab=VSS}
N 1260 -870 1260 -830 {lab=VDD}
N 1480 -710 1490 -710 {
lab=Vout8I}
N 1140 -590 1140 -550 {lab="v0, v0"}
N 1130 -870 1130 -830 {lab="v0, v0, v0, v0, v0, v0, v0, v0"}
N 1010 -390 1040 -390 {
lab=VoutI[1]}
N 1010 -310 1040 -310 {
lab=VoutQ[1]}
N 1010 -350 1040 -350 {
lab=VoutIB[1]}
N 1010 -270 1040 -270 {
lab=VoutQB[1]}
N 1260 -200 1260 -160 {lab=VSS}
N 1260 -480 1260 -440 {lab=VDD}
N 1480 -320 1490 -320 {
lab=Vout4I4Q}
N 1140 -200 1140 -160 {lab="v0, v0"}
N 1130 -480 1130 -440 {lab="v1, v1, v1, v1, v0, v0, v0, v0"}
N 1740 -780 1770 -780 {
lab=VoutI[2]}
N 1740 -700 1770 -700 {
lab=VoutQ[2]}
N 1740 -740 1770 -740 {
lab=VoutIB[2]}
N 1740 -660 1770 -660 {
lab=VoutQB[2]}
N 1990 -590 1990 -550 {lab=VSS}
N 1990 -870 1990 -830 {lab=VDD}
N 2210 -710 2220 -710 {
lab=Vout8Q}
N 1870 -590 1870 -550 {lab="v0, v0"}
N 1860 -870 1860 -830 {lab="v1, v1, v1, v1, v1, v1, v1, v1"}
N 1770 -390 1800 -390 {
lab=VoutI[3]}
N 1770 -310 1800 -310 {
lab=VoutQ[3]}
N 1770 -350 1800 -350 {
lab=VoutIB[3]}
N 1770 -270 1800 -270 {
lab=VoutQB[3]}
N 2020 -200 2020 -160 {lab=VSS}
N 2020 -480 2020 -440 {lab=VDD}
N 2240 -320 2250 -320 {
lab=Vout5I3Q}
N 1900 -200 1900 -160 {lab="v0, v0"}
N 1890 -480 1890 -440 {lab="v1, v1, v1, v0, v0, v0, v0, v0"}
N 2500 -370 2530 -370 {
lab=VoutI[4]}
N 2500 -290 2530 -290 {
lab=VoutQ[4]}
N 2500 -330 2530 -330 {
lab=VoutIB[4]}
N 2500 -250 2530 -250 {
lab=VoutQB[4]}
N 2750 -180 2750 -140 {lab=VSS}
N 2750 -460 2750 -420 {lab=VDD}
N 2970 -300 2980 -300 {
lab=Vout3I5Q}
N 2630 -180 2630 -140 {lab="v0, v0"}
N 2620 -460 2620 -420 {lab="v1, v1, v1, v1, v1, v0, v0, v0"}
N 1480 -710 1480 -680 {lab=Vout8I}
N 1460 -710 1480 -710 {
lab=Vout8I}
N 1480 -620 1480 -580 {lab=VSS}
N 2210 -710 2210 -680 {lab=Vout8Q}
N 2210 -620 2210 -580 {lab=VSS}
N 2190 -710 2210 -710 {
lab=Vout8Q}
N 1480 -320 1480 -290 {lab=Vout4I4Q}
N 1480 -230 1480 -190 {lab=VSS}
N 1460 -320 1480 -320 {
lab=Vout4I4Q}
N 2240 -320 2240 -290 {lab=Vout5I3Q}
N 2240 -230 2240 -190 {lab=VSS}
N 2220 -320 2240 -320 {
lab=Vout5I3Q}
N 2970 -300 2970 -270 {lab=Vout3I5Q}
N 2970 -210 2970 -170 {lab=VSS}
N 2950 -300 2970 -300 {
lab=Vout3I5Q}
C {8xPI/8xPI.sym} 1020 -570 0 0 {name=x1}
C {devices/gnd.sym} 120 -480 0 0 {name=l2 lab=GND}
C {devices/vsource.sym} 120 -530 0 0 {name=Vin3 value="dc 0 ac 0 pulse(0, 1.2, 0, 25p, 25p, 225p, 500p) "}
C {devices/lab_pin.sym} 100 -620 0 0 {name=p1 sig_type=std_logic lab=vinI}
C {devices/lab_pin.sym} 100 -430 0 0 {name=p2 sig_type=std_logic lab=vinQ}
C {devices/vsource.sym} 120 -340 0 0 {name=Vin4 value="dc 0 ac 0 pulse(0, 1.2, 125p, 25p, 25p, 225p, 500p ) "}
C {devices/gnd.sym} 440 -480 0 0 {name=l4 lab=GND}
C {devices/vsource.sym} 440 -530 0 0 {name=Vin5 value="dc 0 ac 0 pulse(0, 1.2, 250p, 25p, 25p, 225p, 500p ) "}
C {devices/lab_pin.sym} 420 -620 0 0 {name=p3 sig_type=std_logic lab=vinIB}
C {devices/vsource.sym} 440 -340 0 0 {name=Vin6 value="dc 0 ac 0 pulse(0, 1.2, 375p, 25p, 25p, 225p, 500p ) "}
C {devices/lab_pin.sym} 420 -430 0 0 {name=p6 sig_type=std_logic lab=vinQB}
C {4to4/4to4.sym} 300 -140 0 0 {name=x2[4:0]}
C {devices/lab_pin.sym} 300 -260 0 0 {name=p4 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} 130 -170 0 0 {name=p14 sig_type=std_logic lab=vinI}
C {devices/lab_pin.sym} 130 -150 0 0 {name=p8 sig_type=std_logic lab=vinQ}
C {devices/lab_pin.sym} 130 -130 0 0 {name=p17 sig_type=std_logic lab=vinIB}
C {devices/lab_pin.sym} 130 -110 0 0 {name=p18 sig_type=std_logic lab=vinQB}
C {devices/lab_pin.sym} 480 -170 2 0 {name=p19 sig_type=std_logic lab=VoutI[4:0]}
C {devices/lab_pin.sym} 480 -150 2 0 {name=p20 sig_type=std_logic lab=VoutQ[4:0]}
C {devices/lab_pin.sym} 480 -130 2 0 {name=p21 sig_type=std_logic lab=VoutIB[4:0]}
C {devices/lab_pin.sym} 480 -110 2 0 {name=p22 sig_type=std_logic lab=VoutQB[4:0]}
C {devices/gnd.sym} 120 -290 0 0 {name=l3 lab=GND}
C {devices/gnd.sym} 440 -290 0 0 {name=l5 lab=GND}
C {devices/vsource.sym} 320 -760 0 0 {name=Vdd4 value=1.2}
C {devices/lab_pin.sym} 320 -830 0 0 {name=p26 sig_type=std_logic lab=VDD}
C {devices/vsource.sym} 220 -760 0 0 {name=Vdd1 value=1.2}
C {devices/vsource.sym} 120 -760 0 0 {name=Vdd2 value=0}
C {devices/lab_pin.sym} 120 -830 0 0 {name=p11 sig_type=std_logic lab=v0}
C {devices/lab_pin.sym} 220 -830 0 0 {name=p10 sig_type=std_logic lab=v1}
C {devices/vsource.sym} 420 -760 0 0 {name=Vdd3 value=0}
C {devices/gnd.sym} 420 -670 0 0 {name=l1 lab=GND}
C {devices/lab_pin.sym} 420 -830 0 0 {name=p5 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 120 -690 2 0 {name=p7 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 220 -690 2 0 {name=p9 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 320 -690 2 0 {name=p12 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 300 -20 2 0 {name=p13 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 1010 -780 0 0 {name=p15 sig_type=std_logic lab=VoutI[0]}
C {devices/lab_pin.sym} 1010 -700 0 0 {name=p16 sig_type=std_logic lab=VoutQ[0]}
C {devices/lab_pin.sym} 1010 -740 0 0 {name=p23 sig_type=std_logic lab=VoutIB[0]}
C {devices/lab_pin.sym} 1010 -660 0 0 {name=p24 sig_type=std_logic lab=VoutQB[0]}
C {devices/lab_pin.sym} 1260 -550 2 0 {name=p25 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 1260 -870 0 0 {name=p27 sig_type=std_logic lab=VDD}
C {devices/code_shown.sym} 1800 -1590 0 0 {name=NGSPICE only_toplevel=true value="
.param temp=27
.control
save all 
tran 5p 1.5n

*meas tran tPWI TRIG v(Vout_I) VAl=0.6 RISE=1 TARG v(Vout_I) VAl=0.6 FALL=1
*meas tran tPWIQ TRIG v(Vout_IQ) VAl=0.6 RISE=1 TARG v(Vout_IQ) VAl=0.6 FALL=1
*meas tran tPWQ TRIG v(Vout_Q) VAl=0.6 RISE=1 TARG v(Vout_Q) VAl=0.6 FALL=1
*meas tran tPWQIB TRIG v(Vout_QIB) VAl=0.6 RISE=1 TARG v(Vout_QIB) VAl=0.6 FALL=1
*meas tran tPWIB TRIG v(Vout_IB) VAl=0.6 RISE=1 TARG v(Vout_IB) VAl=0.6 FALL=1
*meas tran tPWIBQB TRIG v(Vout_IBQB) VAl=0.6 RISE=1 TARG v(Vout_IBQB) VAl=0.6 FALL=1
*meas tran tPWQB TRIG v(Vout_QB) VAl=0.6 RISE=1 TARG v(Vout_QB) VAl=0.6 FALL=1
*meas tran tPWQBI TRIG v(Vout_QBI) VAl=0.6 RISE=1 TARG v(Vout_QBI) VAl=0.6 FALL=1


*meas tran tskewI_IQ TRIG v(Vout_I) VAl=0.6 RISE=1 TARG v(Vout_IQ) VAl=0.6 RISE=1
*meas tran tskewIQ_Q TRIG v(Vout_IQ) VAl=0.6 RISE=1 TARG v(Vout_Q) VAl=0.6 RISE=1
*meas tran tskewQ_QIB TRIG v(Vout_Q) VAl=0.6 RISE=1 TARG v(Vout_QIB) VAl=0.6 RISE=1
*meas tran tskewQIB_IB TRIG v(Vout_QIB) VAl=0.6 RISE=1 TARG v(Vout_IB) VAl=0.6 RISE=1
*meas tran tskewIB_IBQB TRIG v(Vout_IB) VAl=0.6 RISE=1 TARG v(Vout_IBQB) VAl=0.6 RISE=1
*meas tran tskewIBQB_QB TRIG v(Vout_IBQB) VAl=0.6 RISE=1 TARG v(Vout_QB) VAl=0.6 RISE=1
*meas tran tskewQB_QBI TRIG v(Vout_QB) VAl=0.6 RISE=1 TARG v(Vout_QBI) VAl=0.6 RISE=1
*meas tran tskewQBI_I TRIG v(Vout_QBI) VAl=0.6 RISE=1 TARG v(Vout_I) VAl=0.6 RISE=2


write tran_8XPI.raw
.endc
"}
C {devices/launcher.sym} 1860 -1770 0 0 {name=h5
descr="load waves Ctrl + left click" 
tclcommand="xschem raw_read [file join $netlist_dir tran_8XPI.raw] tran"
}
C {devices/code_shown.sym} 2140 -1840 0 0 {name=MODEL1 only_toplevel=true
format="tcleval( @value )"
value="
.lib cornerMOSlv.lib mos_tt
"}
C {devices/launcher.sym} 1860 -1720 0 0 {name=h1
descr="Simulate" 
tclcommand="xschem save; xschem netlist; xschem simulate"}
C {devices/lab_pin.sym} 1490 -320 2 0 {name=p28 sig_type=std_logic lab=Vout4I4Q}
C {devices/lab_pin.sym} 1140 -550 2 0 {name=p29 sig_type=std_logic lab="v0, v0"}
C {devices/lab_pin.sym} 1130 -870 0 0 {name=p30 sig_type=std_logic lab="v0, v0, v0, v0, v0, v0, v0, v0"}
C {8xPI/8xPI.sym} 1020 -180 0 0 {name=x3}
C {devices/lab_pin.sym} 1010 -390 0 0 {name=p31 sig_type=std_logic lab=VoutI[1]}
C {devices/lab_pin.sym} 1010 -310 0 0 {name=p32 sig_type=std_logic lab=VoutQ[1]}
C {devices/lab_pin.sym} 1010 -350 0 0 {name=p33 sig_type=std_logic lab=VoutIB[1]}
C {devices/lab_pin.sym} 1010 -270 0 0 {name=p34 sig_type=std_logic lab=VoutQB[1]}
C {devices/lab_pin.sym} 1260 -160 2 0 {name=p35 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 1260 -480 0 0 {name=p36 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} 1490 -710 2 0 {name=p37 sig_type=std_logic lab=Vout8I}
C {devices/lab_pin.sym} 1140 -160 2 0 {name=p38 sig_type=std_logic lab="v0, v0"}
C {devices/lab_pin.sym} 1130 -480 0 0 {name=p39 sig_type=std_logic lab="v1, v1, v1, v1, v0, v0, v0, v0"}
C {8xPI/8xPI.sym} 1750 -570 0 0 {name=x4}
C {devices/lab_pin.sym} 1740 -780 0 0 {name=p40 sig_type=std_logic lab=VoutI[2]}
C {devices/lab_pin.sym} 1740 -700 0 0 {name=p41 sig_type=std_logic lab=VoutQ[2]}
C {devices/lab_pin.sym} 1740 -740 0 0 {name=p42 sig_type=std_logic lab=VoutIB[2]}
C {devices/lab_pin.sym} 1740 -660 0 0 {name=p43 sig_type=std_logic lab=VoutQB[2]}
C {devices/lab_pin.sym} 1990 -550 2 0 {name=p44 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 1990 -870 0 0 {name=p45 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} 2220 -710 2 0 {name=p46 sig_type=std_logic lab=Vout8Q}
C {devices/lab_pin.sym} 1870 -550 2 0 {name=p47 sig_type=std_logic lab="v0, v0"}
C {devices/lab_pin.sym} 1860 -870 0 0 {name=p48 sig_type=std_logic lab="v1, v1, v1, v1, v1, v1, v1, v1"}
C {devices/lab_pin.sym} 2250 -320 2 0 {name=p49 sig_type=std_logic lab=Vout5I3Q}
C {8xPI/8xPI.sym} 1780 -180 0 0 {name=x5}
C {devices/lab_pin.sym} 1770 -390 0 0 {name=p50 sig_type=std_logic lab=VoutI[3]}
C {devices/lab_pin.sym} 1770 -310 0 0 {name=p51 sig_type=std_logic lab=VoutQ[3]}
C {devices/lab_pin.sym} 1770 -350 0 0 {name=p52 sig_type=std_logic lab=VoutIB[3]}
C {devices/lab_pin.sym} 1770 -270 0 0 {name=p53 sig_type=std_logic lab=VoutQB[3]}
C {devices/lab_pin.sym} 2020 -160 2 0 {name=p54 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 2020 -480 0 0 {name=p55 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} 1900 -160 2 0 {name=p56 sig_type=std_logic lab="v0, v0"}
C {devices/lab_pin.sym} 1890 -480 0 0 {name=p57 sig_type=std_logic lab="v1, v1, v1, v0, v0, v0, v0, v0"}
C {devices/lab_pin.sym} 2980 -300 2 0 {name=p58 sig_type=std_logic lab=Vout3I5Q}
C {8xPI/8xPI.sym} 2510 -160 0 0 {name=x6}
C {devices/lab_pin.sym} 2500 -370 0 0 {name=p59 sig_type=std_logic lab=VoutI[4]}
C {devices/lab_pin.sym} 2500 -290 0 0 {name=p60 sig_type=std_logic lab=VoutQ[4]}
C {devices/lab_pin.sym} 2500 -330 0 0 {name=p61 sig_type=std_logic lab=VoutIB[4]}
C {devices/lab_pin.sym} 2500 -250 0 0 {name=p62 sig_type=std_logic lab=VoutQB[4]}
C {devices/lab_pin.sym} 2750 -140 2 0 {name=p63 sig_type=std_logic lab=VSS}
C {devices/lab_pin.sym} 2750 -460 0 0 {name=p64 sig_type=std_logic lab=VDD}
C {devices/lab_pin.sym} 2630 -140 2 0 {name=p65 sig_type=std_logic lab="v0, v0"}
C {devices/lab_pin.sym} 2620 -460 0 0 {name=p66 sig_type=std_logic lab="v1, v1, v1, v1, v1, v0, v0, v0"}
C {capa.sym} 1480 -650 0 0 {name=C1
m=1
value=25f
footprint=1206
device="ceramic capacitor"}
C {devices/lab_pin.sym} 1480 -580 2 0 {name=p67 sig_type=std_logic lab=VSS}
C {capa.sym} 2210 -650 0 0 {name=C2
m=1
value=25f
footprint=1206
device="ceramic capacitor"}
C {devices/lab_pin.sym} 2210 -580 2 0 {name=p68 sig_type=std_logic lab=VSS}
C {capa.sym} 1480 -260 0 0 {name=C3
m=1
value=25f
footprint=1206
device="ceramic capacitor"}
C {devices/lab_pin.sym} 1480 -190 2 0 {name=p69 sig_type=std_logic lab=VSS}
C {capa.sym} 2240 -260 0 0 {name=C4
m=1
value=25f
footprint=1206
device="ceramic capacitor"}
C {devices/lab_pin.sym} 2240 -190 2 0 {name=p70 sig_type=std_logic lab=VSS}
C {capa.sym} 2970 -240 0 0 {name=C5
m=1
value=25f
footprint=1206
device="ceramic capacitor"}
C {devices/lab_pin.sym} 2970 -170 2 0 {name=p71 sig_type=std_logic lab=VSS}
